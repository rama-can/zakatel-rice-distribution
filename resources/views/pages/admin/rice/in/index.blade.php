<x-back-layout>
    @section('title'){{ __('Rice In') }}@endsection
    @push('styles')
    @livewireStyles
    @endpush
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <nav class="text-gray-500 mb-8">
            <ol class="list-none p-0 inline-flex">
                ...
                <ul class="inline-flex flex-wrap text-sm font-medium">
                    ...
                    <li class="flex items-center">
                        <a class="text-indigo-500" href="#0">{{ __('Rice In') }}</a>
                    </li>
                </ul>
                <!-- End -->
            </ol>
        ...
        <div class="bg-white dark:bg-slate-900 shadow-lg rounded-sm border border-gray-200 dark:border-gray-800">
            <header class="px-5 py-4 flex items-center justify-between border-b dark:border-gray-800">
                <h2 class="font-semibold text-gray-800 dark:text-gray-200">All the rice goes in <span class="text-gray-400 font-medium"></span>
                </h2>
                <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
                    <a href="{{ route('rice-in.create') }}"
                        class="btn bg-indigo-600 hover:bg-indigo-700 text-white">
                        <svg class="w-4 h-4 fill-current opacity-50 shrink-0" viewBox="0 0 16 16">
                            <path
                                d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                        </svg>
                        <span class="hidden xs:block ml-2">{{ __('Add Rice') }}</span>
                    </a>
                </div>
            </header>
            <!-- Table -->
            <div class="mt-2 px-4 my-4">
                <div x-data="{ showModal: false, getId: null}"
                    role="dialog"
                    tabindex="-1"
                    x-cloak
                    x-transition
                >
                    <livewire:rice-in-table />
                     <!-- Modal -->
                                ...
                                <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                    <div class="bg-white dark:bg-slate-800 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                                        <div class="sm:flex sm:items-start">
                                            ...
                                            <div class="mt-2">
                                                <p class="text-sm text-gray-500 dark:text-slate-200">Are you sure you want to delete this record? All of your data will be permanently removed. This action cannot be undone.</p>
                                            </div>
                                            </div>
                                        </div>
                                        </div>
                                        <div class="bg-gray-100 dark:bg-slate-700 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                            <form :action="'{{ route('rice-in.destroy', '') }}' + '/' + getId" method="POST">
                                                @csrf
                                                @method('DELETE')
