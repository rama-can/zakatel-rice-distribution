        ...
        <div class="my-8">
            <x-admin.form-section-editor
                ...
                <x-slot name="form">
                    @csrf
                    @method('PUT')
                    <input hidden type="text" name="id" value="{{ $page->id }}">
                    <div class="col-span-6 sm:col-span-8">
                        <div x-data="{ src: '{{ Storage::url($page->image) }}', showInfo: false }" class="mt-2 max-w-sm p-6 mb-4 bg-slate-100 dark:bg-slate-900 border-dashed border-2 border-gray-400 dark:border-gray-500 rounded-lg items-center mx-auto text-center">
                                <div>
                                    <figure>
                                    ...
                                    <label for="image">
                                        <input type="file" class="hidden" id="image" name="image" value="{{ $page->image }}" @change="src = URL.createObjectURL($event.target.files[0]); showInfo = false;">
                    ...
                    <div class="col-span-6 sm:col-span-6">
                        ...
                        <x-text-input id="title" type="text" class="mt-1 block w-full" name="title"
                            :value="old('title', $page->title)" placeholder="title" autocomplete="title" />
                        <x-input-error :messages="$errors->get('title')" class="mt-1" />
                    </div>

                    <div class="col-span-6 sm:col-span-6">
                        <x-input-label for="description" value="{{ __('Description') }}" />
                        <x-input-text-area class="mt-1 w-full" id="description" name="description" placeholder="Description" cols="4" rows="3" :value="old('description', $page->description)" />
                        <x-input-error :messages="$errors->get('description')" class="mt-1" />
                    </div>

                    <div class="col-span-6 sm:col-span-3">
                        <x-input-label for="status" value="{{ __('Status') }}" />
                        <select id="status" name="status" class="mt-1 w-full border-slate-300 bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" x-data="{ status: '{{ $page->status }}'}">
                            <option value="" disabled selected>Choose Option</option>
                            <option value="draft" x-bind:selected="status === 'draft'">Draft</option>
                            <option value="published" x-bind:selected="status === 'published'">Published</option>
                        </select>
